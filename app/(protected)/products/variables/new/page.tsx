...
import { createPricingVariableAction } from "@/modules/products/catalog-actions";

interface NewPricingVariablePageProps {
  searchParams: Promise<{ error?: string }>;
}

export default async function NewPricingVariablePage({ searchParams }: NewPricingVariablePageProps) {
    ...
    <section className={`${ERP_SHEET_WRAPPER_CLASSNAME} p-3 lg:p-4`}>
      <div className={`${ERP_SHEET_CLASSNAME} px-5 py-4`}>
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--tazki-slate-200)] pb-3">
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--tazki-slate-950)]">Nueva variable</h1>
        ...
        <div className="pt-4">
          ...
          <form action={createPricingVariableAction} className="space-y-5">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="space-y-2">
                ...
                <select name="variable_type" defaultValue="number" className="h-10 w-full rounded-lg border border-[var(--tazki-slate-200)] bg-white px-3 text-sm">
              ...
              <label className="space-y-2">
                ...
                <input name="unit" className="h-10 w-full rounded-lg border border-[var(--tazki-slate-200)] bg-white px-3 text-sm" />
              </label>
            </div>
