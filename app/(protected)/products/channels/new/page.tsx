...
import { createSalesChannelAction } from "@/modules/products/catalog-actions";

interface NewSalesChannelPageProps {
  searchParams: Promise<{ error?: string }>;
}

export default async function NewSalesChannelPage({ searchParams }: NewSalesChannelPageProps) {
    ...
    <section className={`${ERP_SHEET_WRAPPER_CLASSNAME} p-3 lg:p-4`}>
      <div className={`${ERP_SHEET_CLASSNAME} px-5 py-4`}>
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--tazki-slate-200)] pb-3">
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--tazki-slate-950)]">Nuevo canal</h1>
        ...
        <div className="pt-4">
          ...
          <form action={createSalesChannelAction} className="space-y-5">
