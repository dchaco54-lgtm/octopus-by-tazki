...
import { moveColumn, orderColumnOptions } from "@/modules/billing/list-view/helpers";
import { ToolbarDropdown } from "@/modules/subscriptions/list-view/toolbar-dropdown";

interface ColumnVisibilityMenuProps {
  columns: BillingColumnKey[];
  onChange: (columns: BillingColumnKey[]) => void;
  onResetWidths: () => void;
}
...
export function ColumnVisibilityMenu({ columns, onChange, onResetWidths }: ColumnVisibilityMenuProps) {
  ...
  function toggleColumn(column: BillingColumnKey, checked: boolean) {
      ...
      {({ close }) => (
        <div className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--tazki-slate-500)]">La vista base mantiene las columnas operativas obligatorias.</p>
          </div>
